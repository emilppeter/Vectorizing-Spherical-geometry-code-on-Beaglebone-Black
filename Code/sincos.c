...
extern double const twopi=2.0*DP_PI;			// pi times 2
extern double const two_over_pi= 2.0/DP_PI;		// 2/pi
...
#endif
extern const uint32_t case1[]={0,0,0,0};
extern const uint32_t case2[]={1,1,1,1};
extern const uint32_t case3[]={2,2,2,2};
extern const uint32_t case4[]={3,3,3,3};
float cos_12s(float x)
{
const float c1= 0.94560307;
...
float cos_12(float x){
	...
	if((int)(x * two_over_pi)==0)
		return cos_12s(x);
	else if ((int)(x * two_over_pi)==1)
		return -cos_12s(DP_PI-x);
	else if ((int)(x * two_over_pi)==2)
		return -cos_12s(x-DP_PI);
	else if ((int)(x * two_over_pi)==3)
		return  cos_12s(twopi-x);
  return 0.0;
}
float32x4_t v_cos_12(float32x4_t v4_x){
	...
	v4_quad=vcvtq_u32_f32(vmulq_n_f32(v4_x,two_over_pi));
	v4_case2=vsubq_f32(vdupq_n_f32(DP_PI),v4_x);
	v4_case3=vsubq_f32(v4_x,vdupq_n_f32(DP_PI));
	v4_case4=vsubq_f32(vdupq_n_f32(twopi),v4_x);
	
	v4_compare=vceqq_u32(vld1q_u32(case1),v4_quad);
	v4_result=vbslq_f32(v4_compare,v4_x,v4_result);
		
	v4_compare=vceqq_u32(vld1q_u32(case2),v4_quad);
	v4_compare_neg1=v4_compare;
	v4_result=vbslq_f32(v4_compare,v4_case2,v4_result);
	
	v4_compare=vceqq_u32(vld1q_u32(case3),v4_quad);
	v4_compare_neg2=v4_compare;
	v4_result=vbslq_f32(v4_compare,v4_case3,v4_result);
	
	v4_compare=vceqq_u32(vld1q_u32(case4),v4_quad);
	...
	v4_result=vbslq_f32(v4_compare_neg2,v4_result_temp,v4_result);
	return v4_result;
}
// *********************************************************
// ***
...
float cos_32(float x){
	...
	if(quad==0)
		...
		return -cos_32s(DP_PI-x);
	else if (quad==2)
		return -cos_32s(x-DP_PI);
	else if (quad==3)
...
float cos_52(float x){
	...
	if(quad==0)
		...
		return -cos_52s(DP_PI-x);
	else if (quad==2)
		return -cos_52s(x-DP_PI);
	else if (quad==3)
...
//
inline double cos_73s(double x)
{
...
inline double cos_73(double x){
	...
	if(quad==0)
		...
		return -cos_73s(DP_PI-x);
	else if (quad==2)
		return -cos_73s(x-DP_PI);
	else if (quad==3)
...
float32x4_t v_cos_73(float32x4_t v4_x){
	...
	v4_result=vmulq_f32(v4_result,v4_result);
	v4_term=vmulq_n_f32(v4_result,c5);
	v4_term=vaddq_f32(vdupq_n_f32(c4),v4_term);
	v4_term=vmulq_f32(v4_result,v4_term);
	v4_term=vaddq_f32(vdupq_n_f32(c3),v4_term);
	v4_term=vmulq_f32(v4_result,v4_term);
	v4_term=vaddq_f32(vdupq_n_f32(c2),v4_term);
	v4_term=vmulq_f32(v4_result,v4_term);
	v4_result=vaddq_f32(vdupq_n_f32(c1),v4_term);
...
double cos_121(double x){
	...
	if(quad==0)
		...
		return -cos_121s(DP_PI-x);
	else if (quad==2)
		return -cos_121s(x-DP_PI);
	else if (quad==3)
